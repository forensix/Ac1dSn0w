...
// forensix: Use unused attribute to avoid compiler warning.
static struct irecv_device __attribute__((unused)) irecv_devices[] = {
...
// forensix: Avoid compiler warning.
...
// forensix: limera1n darwin patch.
int irecv_short_time_control_transfer(irecv_client_t client, uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, unsigned char *data, uint16_t wLength);

// forensix: USB over IP support.
